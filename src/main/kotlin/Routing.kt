...
fun Application.configureRouting() {
    ...
    val toyotaService: ToyotaService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Josua Saragih (11S23028).")
        }
        ...
        route("/toyotas") {
            get {
                toyotaService.getAllToyotas(call)
            }
            post {
                toyotaService.createToyota(call)
            }
            get("/{id}") {
                toyotaService.getToyotaById(call)
            }
            put("/{id}") {
                toyotaService.updateToyota(call)
            }
            delete("/{id}") {
                toyotaService.deleteToyota(call)
            }
            get("/{id}/image") {
                toyotaService.getToyotaImage(call)
            }
        }
